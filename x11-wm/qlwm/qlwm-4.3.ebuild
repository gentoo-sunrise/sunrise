...
src_unpack() {
	...
	# fix path to Qt4's qmake
	sed -i -e 's!^QMAKE .*$!QMAKE = /usr/bin/qmake!' "Makefile" \
	   || die "Cannot fix Makefile variable for qmake"
	# fixing path/filename to qtconfig
	sed -i -e 's!qtconfig-qt4!/usr/bin/qtconfig!' "files/menuconfig" \
